...
[void]$sb.AppendLine()
[void]$sb.AppendLine("Status from latest completed workflow runs (shields.io). Merge successes: **$successMerges** / $($repoJobs.Count).")
[void]$sb.AppendLine()
...
foreach ($job in $repoJobs) {
    ...
    # Landing page: show green success badges; hide stale PR/release failures (common
    # after workflow renames). Merge always shows real conclusion when present.
...
[void]$sb.AppendLine()
[void]$sb.AppendLine("Latest **GitHub Packages** versions (green shields) and **nuget.org** when published. Feed: ``https://nuget.pkg.github.com/$Org/index.json``. Full catalog: [org packages](https://github.com/orgs/$Org/packages) · [novolis-registry](https://github.com/$Org/novolis-registry).")
[void]$sb.AppendLine()
...
foreach ($p in ($packages | Sort-Object name)) {
    ...
    $gprCell = Get-VersionShield -Version $gprVer -Href $pkgUrl -Label 'GPR'

    $nugetVer = if ($nugetOrgMap.ContainsKey($pkgName)) { $nugetOrgMap[$pkgName] } else { '' }
    if ($nugetVer) {
        $nugetCell = Get-VersionShield -Version $nugetVer -Href "https://www.nuget.org/packages/$pkgName" -Label 'nuget.org'
    }
    else {
        $nugetCell = '—'
    }
