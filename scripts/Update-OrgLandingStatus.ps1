...
function Get-StatusShield {
    param(
        ...
        [string] $WorkflowFile,
        [string] $Label,
        [string] $Conclusion  # retained for callers / merge-success counts; not baked into the image
    )
    if (-not $WorkflowFile) { return '—' }
    $href = "https://github.com/$Org/$Repo/actions/workflows/$WorkflowFile"
    # Live GitHub Actions badge (updates without regenerating the README).
    # Merge is scoped to main; PR/release use the workflow's default badge query.
    $badgeQuery = if ($WorkflowFile -eq 'merge.yml') { '?branch=main' } else { '' }
    $badge = "https://github.com/$Org/$Repo/actions/workflows/$WorkflowFile/badge.svg$badgeQuery"
...
function Get-RepoBadges {
    ...
    # Show real release conclusion when present (success = green; failure kept for release table honesty)
    $releaseBadge = if ($Job.Release -and $releaseConc) {
        Get-StatusShield -Repo $name -WorkflowFile $Job.Release -Label 'release' -Conclusion $releaseConc
    } elseif ($Job.Release) { '—' } else { '—' }

    return [pscustomobject]@{
        Name         = $name
        Link         = "[``$name``](https://github.com/$Org/$name)"
        Pr           = $prBadge
        Merge        = $mergeBadge
        Release      = $releaseBadge
        HasPackages  = $packagesByRepo.ContainsKey($name) -and @($packagesByRepo[$name]).Count -gt 0
        HasReleaseWf = [bool]$Job.Release
    }
}
...
[void]$sb.AppendLine()
[void]$sb.AppendLine("CI badges are **live** GitHub Actions SVGs. Package versions and the merge-success count below are snapshotted at regen ($generatedAt).")
[void]$sb.AppendLine()
[void]$sb.AppendLine("Merge successes (at last regen): **$successMerges** / $($repoJobs.Count). NuGet packages on GPR: **$($packages.Count)** · [org packages](https://github.com/orgs/$Org/packages) · [novolis-registry](https://github.com/$Org/novolis-registry).")
[void]$sb.AppendLine()

# --- 1. Packages (big table) ---
[void]$sb.AppendLine('### Packages')
[void]$sb.AppendLine()
[void]$sb.AppendLine("Repos that publish to GitHub Packages (top **$MaxPackagesPerRepo** package IDs + count).")
[void]$sb.AppendLine()
[void]$sb.AppendLine('| Repository | PR | Merge | Packages |')
[void]$sb.AppendLine('|------------|----|-------|----------|')
foreach ($row in $packageRepos) {
    $pkgs = @($packagesByRepo[$row.Name])
    $pkgCell = Format-PackageCell -Pkgs $pkgs -RepoName $row.Name
    [void]$sb.AppendLine("| $($row.Link) | $($row.Pr) | $($row.Merge) | $pkgCell |")
}
...
if ($packageRepos.Count -eq 0 -and $orphanPackages.Count -eq 0) {
    [void]$sb.AppendLine('| — | — | — | — |')
}
...
[void]$sb.AppendLine('|------------|----|-------|')
if ($otherRepos.Count -eq 0) {
    [void]$sb.AppendLine('| — | — | — |')
}
else {
    foreach ($row in $otherRepos) {
        [void]$sb.AppendLine("| $($row.Link) | $($row.Pr) | $($row.Merge) |")
    }
}
...
[void]$sb.AppendLine('|------------|---------|')
if ($releaseRepos.Count -eq 0) {
    [void]$sb.AppendLine('| — | — |')
}
else {
    foreach ($row in $releaseRepos) {
        [void]$sb.AppendLine("| $($row.Link) | $($row.Release) |")
    }
}
...
Write-Host "  repos=$($repos.Count) packages=$($packages.Count) merge_success=$successMerges package_repos=$($packageRepos.Count) other=$($otherRepos.Count) release_repos=$($releaseRepos.Count)"
